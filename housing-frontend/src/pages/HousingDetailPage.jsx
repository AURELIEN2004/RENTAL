...
const HousingDetailPage = () => {
  ...
  const handlePlanVisit = async (e) => {
    e.preventDefault();
    if (!user) {
      toast.error(t('loginRequired') || 'Connectez-vous pour planifier une visite');
      navigate('/login');
      return;
    }
    setVisitLoading(true);
    ...
    <div className="housing-detail-page">
      ...
      <div className="container">
        ...
        <div className="gallery-section">
          <div className="main-image" style={{ position: 'relative', cursor: 'zoom-in' }} onClick={() => setLightbox(true)}>
            <img
              src={imgUrl(housing.images?.[selectedImage]?.image) || '/placeholder.jpg'}
              alt={housing.title}
      ...
      {lightbox && (
        ...
        >
          ...
          <img
            src={imgUrl(housing.images?.[selectedImage]?.image)}
            alt={housing.title}
            onClick={e => e.stopPropagation()}
          />
          {housing.images?.length > 1 && (
      ...
      {showVisitModal && (
        <div className="modal-overlay" onClick={closeVisitModal}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="hdp-modal-head">
              <h2><FaCalendarAlt style={{ marginRight: 8, color: '#10B981' }} />{t('planVisit') || 'Planifier une visite'}</h2>
              <button className="hdp-modal-close" onClick={closeVisitModal}>
                <FaTimes />
              </button>
            </div>
            ...
            <div className="hdp-modal-housing-recap">
              <img
                src={imgUrl(housing.images?.[0]?.image) || '/placeholder.jpg'}
                alt={housing.title}
                onError={e => { e.target.src = '/placeholder.jpg'; }}
              />
              <div>
                <strong>{housing.title}</strong>
                <p><FaMapMarkerAlt style={{ fontSize: 10, marginRight: 4 }} />
                  {[housing.district?.name, housing.city?.name].filter(Boolean).join(', ')}
                </p>
                <p style={{ color: '#F59E0B', fontWeight: 700 }}>{formatPrice(housing.price)}/mois</p>
              </div>
            </div>
