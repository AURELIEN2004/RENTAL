...
const T = {
  fr: {
    ...
    view_list:'Liste', view_map:'Carte', view_split:'Mixte',
    on_map:'sur la carte', avg_price:'Prix moyen', min_price:'À partir de',
    filters:'Filtres', reinit:'Réinitialiser', apply:'Appliquer les filtres', close:'Fermer',
    category:'Catégorie', all_cat:'Tous les types',
    type:'Type de logement', all_types:'Tous les types',
    price:'Prix (FCFA)', price_min:'Min', price_max:'Max',
    surface:'Surface (m²)', surf_min:'min', surf_max:'500+',
  ...
  en: {
    ...
    view_list:'List', view_map:'Map', view_split:'Split',
    on_map:'on map', avg_price:'Avg price', min_price:'From',
    filters:'Filters', reinit:'Reset', apply:'Apply filters', close:'Close',
    category:'Category', all_cat:'All types',
    type:'Housing type', all_types:'All types',
    price:'Price (FCFA)', price_min:'Min', price_max:'Max',
    surface:'Surface (m²)', surf_min:'min', surf_max:'500+',
...
// Sidebar Filtres intégrée (pas de popup)
// ════════════════════════════════════════════════════════════
function SidebarFilters({ t, onApply, onReset, filters, setFilters, open, onClose }) {
  const [categories,   setCategories]   = useState([]);
  const [housingTypes, setHousingTypes] = useState([]);
  const [regions,      setRegions]      = useState([]);
  const [cities,       setCities]       = useState([]);
  ...
  return (
    <aside className={`sp-sidebar${open ? ' sp-sidebar--open' : ''}`}>
      <div className="sp-sidebar-head">
        <span className="sp-sidebar-title">
          <SlidersHorizontal size={15}/> {t.filters}
          {activeCount > 0 && <span className="sf-active-count">{activeCount}</span>}
        </span>
        <div className="sp-sidebar-head-actions">
          <button className="sp-sidebar-reinit" onClick={onReset}>{t.reinit}</button>
          {onClose && (
            <button className="sp-sidebar-close" onClick={onClose} aria-label={t.close || 'Fermer'}>
              <X size={17}/>
            </button>
          )}
        </div>
      ...
      <button className="sf-apply-btn" onClick={() => { onApply(); onClose?.(); }}>{t.apply}</button>
    </aside>
  );
}
...
const SearchPage = () => {
  ...
  const [sortBy,       setSortBy]       = useState('recent');
  const [filters,      setFilters]      = useState({...EMPTY_FILTERS});
  const [filtersOpen,  setFiltersOpen]  = useState(false); // panneau filtres mobile (fermé par défaut)
  ...
  // Empêche le scroll de la page derrière le panneau filtres mobile
  useEffect(() => {
  ...
  return (
    <div className="search-page sp2">
        ...
        <div className="sp2-header-inner">
          ...
          <div className="sp2-toolbar">
            ...
            <div className="sp2-right-ctrl">
              ...
              <button className="sp-filters-toggle-btn" onClick={() => setFiltersOpen(true)}>
                <SlidersHorizontal size={14}/> {t.filters}
                {(() => {
                  const n = Object.entries(filters).filter(
                    ([k, v]) => v !== '' && v != null && v !== 0 && !(k === 'status' && v === 'disponible')
                  ).length;
                  return n > 0 ? <span className="sp-filters-toggle-count">{n}</span> : null;
                })()}
              </button>
              ...
              {/* Vue toggle */}
              <div className="sp2-view-toggle">
                <button className={`sp2-vbtn${viewMode==='list'?' sp2-vbtn--act':''}`} onClick={()=>setViewMode('list')} title={t.view_list} aria-label={t.view_list} aria-pressed={viewMode==='list'}>
                  <i className="fas fa-grid-2"/>
                </button>
                <button className={`sp2-vbtn${viewMode==='map'?' sp2-vbtn--act':''}`} onClick={()=>setViewMode('map')} title={t.view_map} aria-label={t.view_map} aria-pressed={viewMode==='map'}>
                  <Map size={14}/>
                </button>
                <button className={`sp2-vbtn${viewMode==='split'?' sp2-vbtn--act':''}`} onClick={()=>setViewMode('split')} title={t.view_split} aria-label={t.view_split} aria-pressed={viewMode==='split'}>
                  <Columns size={14}/>
                </button>
              </div>
            </div>
      ...
      <div className="sp2-body">
        ...
        {/* ── Sidebar Filtres ── */}
        <SidebarFilters
          t={t} filters={filters} setFilters={setFilters}
          onApply={handleApply} onReset={handleReset}
          open={filtersOpen} onClose={() => setFiltersOpen(false)}
        />

        {/* ── Backdrop mobile (visible seulement quand le panneau filtres est ouvert) ── */}
        {filtersOpen && (
          <div className="sp-sidebar-backdrop" onClick={() => setFiltersOpen(false)} />
        )}
