...
class HousingViewSet(viewsets.ModelViewSet):
    ...
    # --- ACTIONS UTILISATEURS (RECOMMANDATIONS & INTERACTIONS) ---

    @action(detail=False, methods=['get'])
    def recommended(self, request):
        """
        Logements recommandés avec algorithme génétique
        ✅ VERSION FUSIONNÉE avec gestion d'erreur et fallback
        """
        try:
            ...
            # Algorithme génétique pour utilisateurs connectés
    ...
    # --- ACTIONS PROPRIÉTAIRES (GESTION & STATS) ---

    @action(detail=False, methods=['get'])
    def my_housings(self, request):
        """Logements du propriétaire connecté avec options de tri et filtres"""
    ...
    def owner_statistics(self, request):
        """Tableau de bord statistique pour le propriétaire"""
        housings = Housing.objects.filter(owner=request.user)
        
        # Import local pour éviter les imports circulaires si nécessaire
